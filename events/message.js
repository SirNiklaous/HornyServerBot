...
module.exports = (client, message) => {
    let dmlogggingchannel = '👃┃dms';
    if (!message.channel.viewable || message.author.bot) return;
    if (message.channel.type === 'dm') {
      dmloggingchannel.send(new MessageEmbed().setAuthor(message.member.displayName, 
      message.author.displayAvatarURL({ dynamic: true }).setDescription(message.content).setFooter(message.member.id))
      )}
  ...
  if (prefixRegex.test(message.content)) {
    ...
    if (command && !disabledCommands.includes(command.name)) {
      ...
      const embed = new MessageEmbed()
        .setTitle('Hi, I\'m Anime Chills. Need help?')
        .setDescription(`You can see everything I can do by using the \`${prefix}help\` command.`)
        .setFooter('DM Polaroid#2226 to speak directly with the developer!')
