module.exports = async (client) => {
  ...
  const activities = [
    ...
    { name: 'Anime Chills', type: 'WATCHING' },
    { name: 'Your Waifus', type: 'WATCHING' }
  ];
  ...
  setInterval(() => {
    activities[2] = { name: `Anime Chills`, type: 'WATCHING' }; // Update server count
  ...
  for (const guild of leftGuilds) {
    ...
    client.logger.info(`Anime Chills has left ${guild.guild_name}`);
  }

  client.logger.info('Anime Chills is now online');
  client.logger.info(`Anime Chills is running on ${client.guilds.cache.size} server(s)`);
};
